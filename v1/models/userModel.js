...
class User extends BaseModel {
    static async createUser (data) {
        ...
        await this.associateSpokenLanguages(result.id, spokenLanguages);
        return result;
    }
    ...
    static async deleteUser (id) {
        ...
        const result = await this.querySingle(query, [id]);
    ...
    static async getUser (id) {
        ...
        const result = await this.querySingle('SELECT * FROM users WHERE id = $1', [id]);

        result.spokenLanguages = await this.fetchSpokenLanguages(id);
        result.boatIds = await this.fetchBoatIds(id);
    ...
    static async getAllUsers (filters) {
        ...
        for (const user of result) {
            user.spokenLanguages = await this.fetchSpokenLanguages(user.id);
            user.boatIds = await this.fetchBoatIds(user.id);
        }
